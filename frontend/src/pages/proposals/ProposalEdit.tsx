  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        ...
        // Gauti pasiūlymo duomenis
        if (id) {
          const proposalResponse = await api.get(`/api/proposals/${id}`);
          ...
          setCircle(proposalData.circle);
          
          if (proposalData.deadline) {
            setDeadline(new Date(proposalData.deadline));
          }
          
          // Nustatyti dalyvius
          if (proposalData.participants && proposalData.participants.length > 0) {
            setParticipants(proposalData.participants);
          }
          
          // Nustatyti facilitatorių
          if (proposalData.facilitator) {
            setFacilitator(proposalData.facilitator);
          }
        }
      } catch (err: any) {
